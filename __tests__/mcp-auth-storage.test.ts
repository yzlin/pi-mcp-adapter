...
describe("OAuth credential-store diagnostics", () => {
  ...
  it("explains Windows network-logon error 1312 without falling back", async () => {
  ...
  it("does not infer the encrypted backend from untrusted OS error text", async () => {
...
describe("encrypted OAuth credential file store", () => {
  ...
  it("selects only the explicit backend and ignores legacy path configuration", async () => {
  ...
  it("round-trips and removes credentials without persisting secrets", async () => {
    const entry = {
    ...
    await saveAuthEntry("secure-server", entry, SERVER_URL, options);
    ...
    await clearAllCredentials("secure-server", options);
    expect(existsSync(path)).toBe(false);
  });

  it("uses fresh ciphertext when credentials are rewritten", async () => {
    await saveAuthEntry("rewrite", { tokens: { accessToken: "secret" } }, SERVER_URL, options);
  ...
  it("does not serve cached credentials after the key changes or disappears", async () => {
    await saveAuthEntry("key-rotation", { tokens: { accessToken: "secret" } }, SERVER_URL, options);
  ...
  it("evicts every cached key generation after removal", async () => {
    delete process.env.PI_MCP_ADAPTER_DISABLE_AUTH_CACHE;
    await saveAuthEntry("remove-generations", { tokens: { accessToken: "secret" } }, SERVER_URL, options);
    expect(getAuthEntry("remove-generations", options)?.tokens?.accessToken).toBe("secret");

    process.env.PI_MCP_ADAPTER_OAUTH_FILE_KEY = rotatedKey;
    await clearAllCredentials("remove-generations", options);
  ...
  it("does not evict a different NUL-containing server name", async () => {
    delete process.env.PI_MCP_ADAPTER_DISABLE_AUTH_CACHE;
    const cachedServer = "x\0bar";
    await saveAuthEntry(cachedServer, { tokens: { accessToken: "cached-secret" } }, SERVER_URL, options);
    ...
    process.env.PI_MCP_ADAPTER_OAUTH_FILE_KEY = rotatedKey;
    await clearAllCredentials("bar", options);
  ...
  it("rejects tampered ciphertext", async () => {
    await saveAuthEntry("tamper", { tokens: { accessToken: "secret" } }, SERVER_URL, options);
  ...
  it("binds ciphertext to its server account", async () => {
    await saveAuthEntry("account-a", { tokens: { accessToken: "secret" } }, SERVER_URL, options);
  ...
  it("preserves URL binding", async () => {
    await saveAuthEntry("url-bound", { tokens: { accessToken: "secret" } }, SERVER_URL, options);
  ...
  it("leaves legacy plaintext untouched", async () => {
  ...
  it("preserves legacy plaintext when encrypted credentials are written", async () => {
    const legacyPath = getAuthEntryFilePath("legacy-write");
    mkdirSync(dirname(legacyPath), { recursive: true });
    writeFileSync(legacyPath, JSON.stringify({ tokens: { accessToken: "legacy" }, serverUrl: SERVER_URL }));

    await saveAuthEntry("legacy-write", { tokens: { accessToken: "encrypted" } }, SERVER_URL, options);

    expect(existsSync(legacyPath)).toBe(true);
    expect(getAuthEntry("legacy-write", options)?.tokens?.accessToken).toBe("encrypted");
  });

  it.runIf(process.platform !== "win32")("creates private files and rejects permissive files", async () => {
    await saveAuthEntry("permissions", { tokens: { accessToken: "secret" } }, SERVER_URL, options);
  ...
  it.runIf(process.platform !== "win32")("rejects regular and dangling symlink destinations for every operation", async () => {
    ...
    for (const operation of ["read", "remove", "write"] as const) {
      ...
      if (operation === "remove") await expect(clearAllCredentials(server, options)).rejects.toThrow(OAuthCredentialStoreError);
      if (operation === "write") await expect(saveAuthEntry(server, { tokens: { accessToken: "secret" } }, SERVER_URL, options)).rejects.toThrow(OAuthCredentialStoreError);
...
describe("mcp-auth storage paths", () => {
  ...
  it("uses configured oauthDir as the legacy import source", async () => {
    ...
    await migrateLegacyAuthEntry("configured", options);
  ...
  it("does not migrate legacy credentials during status-only inspection", async () => {
    ...
    await migrateLegacyAuthEntry("status-only");
  ...
  it("stores an ordinary 9 KiB payload as one native item outside Windows", async () => {
    const accessToken = accessTokenForSerializedLength(9 * 1024);
    await saveAuthEntry("large-entry", { tokens: { accessToken } }, SERVER_URL);
  ...
  it("uses the serialized 1000-character boundary for the size-limited store", async () => {
  ...
  it("keeps surrogate pairs intact across native-string chunk round trips", async () => {
    process.env.PI_MCP_ADAPTER_TEST_AUTH_STORE = "sizelimited";
    ...
    await saveAuthEntry("unicode-boundary", { tokens: { accessToken } }, SERVER_URL);
  ...
  it("persists records that exceed the strictest per-value store limit", async () => {
    process.env.PI_MCP_ADAPTER_TEST_AUTH_STORE = "sizelimited";
    const accessToken = "x".repeat(5000);

    await saveAuthEntry("size-limited-large", { tokens: { accessToken } }, "https://example.com/mcp");
  ...
  it("persists records just above the per-value limit that are too small for a naive chunk threshold", async () => {
    ...
    await saveAuthEntry("size-limited-boundary", { tokens: { accessToken } }, "https://example.com/mcp");

    expect(getAuthEntry("size-limited-boundary")?.tokens?.accessToken).toBe(accessToken);
    expect(getTestAuthSecretStoreEntries().every(([, payload]) => payload.length <= AUTH_SECRET_VALUE_LIMIT)).toBe(true);
  });

  it("keeps small records in a single entry on a size-limited store", async () => {
    process.env.PI_MCP_ADAPTER_TEST_AUTH_STORE = "sizelimited";

    await saveAuthEntry("size-limited-small", { tokens: { accessToken: "small" } }, "https://example.com/mcp");
  ...
  it("clears chunked records written to a size-limited store", async () => {
    process.env.PI_MCP_ADAPTER_TEST_AUTH_STORE = "sizelimited";
    await saveAuthEntry("size-limited-remove", { tokens: { accessToken: "x".repeat(5000) } }, "https://example.com/mcp");
    expect(getTestAuthSecretStoreEntries().some(([account]) => account.includes(".chunk."))).toBe(true);

    await clearAllCredentials("size-limited-remove");
  ...
  it("rejects schema-valid chunk corruption without compacting or deleting it", async () => {
    process.env.PI_MCP_ADAPTER_TEST_AUTH_STORE = "sizelimited";
    await saveAuthEntry("corrupt-chunk", { tokens: { accessToken: "x".repeat(5000) } }, SERVER_URL);
  ...
  it("does not compact chunked records during status-only inspection", async () => {
    process.env.PI_MCP_ADAPTER_TEST_AUTH_STORE = "sizelimited";
    const accessToken = "x".repeat(5000);
    await saveAuthEntry("status-chunks", { tokens: { accessToken } }, "https://example.com/mcp");
    ...
    expect((await migrateLegacyAuthEntry("status-chunks"))?.tokens?.accessToken).toBe(accessToken);
    const compacted = getTestAuthSecretStoreEntries();
    expect(compacted).toHaveLength(1);
    expect(compacted[0][0]).not.toContain(".chunk.");
  });

  it("preserves readable chunks and reports a failed primary compaction write", async () => {
    if (process.platform === "win32") return;
    process.env.PI_MCP_ADAPTER_TEST_AUTH_STORE = "sizelimited";
    const accessToken = "x".repeat(5000);
    await saveAuthEntry("failed-compaction", { tokens: { accessToken } }, SERVER_URL);
    const before = getTestAuthSecretStoreEntries();

    process.env.PI_MCP_ADAPTER_TEST_AUTH_STORE = "writefailing";
    resetAuthEntryCache();
    await expect(migrateLegacyAuthEntry("failed-compaction")).rejects.toThrow(/write OAuth credentials/);
